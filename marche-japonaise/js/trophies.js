...
(function (global) {
  ...
  /* ======================================================================
     MARCHE NORMALE — trophées distincts, adaptés à une marche "classique"
     (sans structure d'intervalles, distance/durée libres). Ajoutés après
     le plafond des 150 trophées "marche japonaise" ci-dessus : ils ne sont
     pas comptés dans ce quota et s'ajoutent en plus.
     ====================================================================== */
  const CAT_NORMALE = "Marche normale";
  CAT.NORMALE = CAT_NORMALE;

  addTier({
    slugBase: "normale-seances",
    cat: CAT_NORMALE,
    emoji: "🚶‍♂️",
    thresholds: [1, 5, 10, 20, 30, 50, 75, 100],
    nameFn: (n) => (n === 1 ? "Première marche normale" : `${n} marches normales`),
    descFn: (n) =>
      n === 1
        ? "Enregistrer votre toute première séance de marche normale."
        : `Enregistrer un total de ${n} séances de marche normale.`,
    valueFn: (ctx) => ctx.countNormale,
  });

  addTier({
    slugBase: "normale-distance-cumulee",
    cat: CAT_NORMALE,
    emoji: "🌍",
    thresholds: [10, 25, 50, 100, 200, 350, 500],
    nameFn: (n) => `${n} km en marche normale`,
    descFn: (n) => `Cumuler ${n} km parcourus en marche normale.`,
    valueFn: (ctx) => ctx.totalDistanceNormale,
  });

  addTier({
    slugBase: "normale-distance-record",
    cat: CAT_NORMALE,
    emoji: "🥾",
    thresholds: [3, 5, 8, 10, 15, 20],
    nameFn: (n) => `${n} km en une marche`,
    descFn: (n) => `Parcourir ${n} km en une seule séance de marche normale — sans contrainte d'intervalles.`,
    valueFn: (ctx) => ctx.bestDistanceNormale,
  });

  addTier({
    slugBase: "normale-vitesse",
    cat: CAT_NORMALE,
    emoji: "💨",
    thresholds: [4, 4.5, 5, 5.5, 6, 7],
    nameFn: (n) => `${String(n).replace(".", ",")} km/h en moyenne`,
    descFn: (n) => `Atteindre une vitesse moyenne de ${String(n).replace(".", ",")} km/h sur une marche normale.`,
    valueFn: (ctx) => ctx.bestVitesseKmh,
  });

  addTier({
    slugBase: "normale-duree",
    cat: CAT_NORMALE,
    emoji: "🕰️",
    thresholds: [30, 45, 60, 90, 120, 180],
    nameFn: (n) => (n < 60 ? `${n} min d'affilée` : `${Math.round(n / 60)} h d'affilée`),
    descFn: (n) => `Marcher ${n < 60 ? n + " minutes" : Math.round(n / 60) + " heure(s)"} d'affilée en marche normale, sans la contrainte des 30 minutes de la marche japonaise.`,
    valueFn: (ctx) => ctx.bestDureeNormaleMin,
  });

  addTier({
    slugBase: "normale-kcal",
    cat: CAT_NORMALE,
    emoji: "🔥",
    thresholds: [500, 1000, 2500, 5000, 10000],
    nameFn: (n) => `${n.toLocaleString("fr-FR")} kcal en marche normale`,
    descFn: (n) => `Cumuler ${n.toLocaleString("fr-FR")} kilocalories sur l'ensemble de vos marches normales.`,
    valueFn: (ctx) => ctx.totalKcalNormale,
  });

  add({
    id: id("normale-polyvalent"),
    cat: CAT_NORMALE,
    emoji: "🔄",
    nom: "Marcheur polyvalent",
    desc: "Avoir enregistré au moins une marche japonaise et une marche normale au cours du même mois.",
    check: (ctx) => !!ctx.hasBothTypesSameMonth,
  });
  add({
    id: id("normale-equilibre"),
    cat: CAT_NORMALE,
    emoji: "⚖️",
    nom: "Le meilleur des deux mondes",
    desc: "Cumuler au moins 10 séances de marche japonaise et 10 séances de marche normale.",
    check: (ctx) => !!ctx.hasTenEachType,
  });
